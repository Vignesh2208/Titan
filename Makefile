...
DISABLE_VT_SOCKET_LAYER := $(shell grep -nr "DISABLE_VT_SOCKET_LAYER" $(HOME)/Titan/CONFIG | awk -F "=" '{print $$2}')
COLLECT_STATS := $(shell grep -nr "COLLECT_STATS" $(HOME)/Titan/CONFIG | awk -F "=" '{print $$2}')


$(info CONFIG USED FOR BUILD:)
...
$(info DISABLE_VT_SOCKET_LAYER=$(DISABLE_VT_SOCKET_LAYER))
$(info COLLECT_STATS=$(COLLECT_STATS))
...
ifeq "$(DISABLE_VT_SOCKET_LAYER)" "yes"
BUILD_CONFIG_FLAGS +=-D'DISABLE_VT_SOCKET_LAYER'
endif
...
ifeq "$(COLLECT_STATS)" "yes"
BUILD_CONFIG_FLAGS +=-D'COLLECT_STATS'
endif
