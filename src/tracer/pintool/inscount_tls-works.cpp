...
*/
s64 send_to_vt_module(ioctl_args *arg) {
...
VOID PIN_FAST_ANALYSIS_CALL docount(UINT32 c, THREADID threadid)
{
    ...
    if (tdata->_curr_clock >= tdata->_target_clock) {
	if (tdata->_vt_enabled) {
		...
		/*
                write_tracer_results(monitorTracerId, &pid, 1);
                tdata->_vt_enabled = 0;
                printf("Disabled VT Advancement !\n");
		*/
		...
		if (tdata->_increment <= 0) {
			...
			tdata->_curr_clock = tdata->_target_clock;
    ...
    /*if (tdata->_vt_enabled) {
    	cout << "Thread Count for Thread: " << threadid << " Count = " << tdata->_curr_clock << endl;
    }*/
    tdata->_curr_clock += c;
...
VOID ThreadStart(THREADID threadid, CONTEXT *ctxt, INT32 flags, VOID *v)
{
    ...
    cout << "Calling ThreadStart for thread: " << threadid << ", PID: " << PIN_GetPid() << endl;
    myPid = PIN_GetTid();
    if (add_processes_to_tracer_sq(monitorTracerId, &myPid, 1) < 0) {
	tdata->_vt_enabled = 0;
        tdata->_curr_clock = 0;
...
int main(int argc, char * argv[])
{
    ...
    tracerClockArray = (s64 *)mmap(NULL, page_size*num_pages, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
