...
void run_processes_in_round(int tracer_id,
							hashmap * tracees,
                            llist * tracee_list,
                            u32 n_round_insns,
                            llist * run_queue,
	...
	do {
		...
		if (n_insns_to_run > 0 && n_insns_run < n_round_insns)
			update_tracer_clock(tracer_id, (s64)n_insns_to_run);
...
int main(int argc, char * argv[]) {
	...
	while ((option = getopt(argc, argv, "i:f:r:sc:h")) != -1) {
		switch (option) {
	...
	if (create_spinner) {
		...
		cpu_assigned = register_tracer(tracer_id, TRACER_TYPE_INS_VT, REGISTRATION_W_SPINNER, spinned_pid);
	} else
		cpu_assigned = register_tracer(tracer_id, TRACER_TYPE_INS_VT, SIMPLE_REGISTRATION, 0);
	...
	while (1) {
		...
		if (num_created) {
			add_processes_to_tracer_sq(tracer_id, created_pids, num_created);
		}

		print_tracee_list(&tracee_list);

		n_round_insns = write_tracer_results(tracer_id, exited_pids, num_exited);
		if (!n_round_insns) {
