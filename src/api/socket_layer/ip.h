...
void IpInit(uint32_t src_ip_addr);

static inline struct iphdr *IpHdr(const struct sk_buff *skb) {
    return (struct iphdr *)(skb->head);
}

static inline uint32_t IpParse(char *addr) {
    uint32_t dst = 0;
