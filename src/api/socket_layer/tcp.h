...
#define tcp_print_hdr(hdr)
#define tcpsock_dbg(msg, sk)
#define tcp_set_state(sk, state)  __TcpSetState(sk, state)
...
int TcpChecksum(struct tcp_sock *sock, struct tcphdr *thdr);
...
static inline int TcpAcceptQueueFull(struct tcp_sock *tsk) {
	return (tsk->accept_backlog >= tsk->backlog);
}

static inline void TcpAcceptEnqueue(struct tcp_sock *tsk) {
	/* move it from listen queue to accept queue */
	if (!ListEmpty(&tsk->list))
		ListDel(&tsk->list);
	ListAdd(&tsk->list, &tsk->parent->accept_queue);
	tsk->accept_backlog++;
}

static inline struct tcp_sock *TcpAcceptDequeue(struct tcp_sock *tsk) {
	struct tcp_sock *newtsk;
	newtsk = list_first_entry(&tsk->accept_queue, struct tcp_sock, list);
	ListDelInit(&newtsk->list);
	tsk->accept_backlog--;
	return newtsk;
}
