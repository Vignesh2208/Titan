...
static void timerfd_show(struct seq_file *m, struct file *file)
{
	...
	seq_printf(m,
		   ...
		   ctx->settime_flags, (unsigned long long)t.it_value.tv_sec,
...
static int do_timerfd_settime(int ufd, int flags, const struct itimerspec *new,
			      struct itimerspec *old)
{
	...
	if ((flags & ~TFD_SETTIME_FLAGS) || !timespec_valid(&new->it_value) ||
