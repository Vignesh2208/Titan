			       ...
			       pid_t pid, uid_t uid, int why, int status)
{
	...
	int retval =
		wo->wo_rusage ? getrusage(p, RUSAGE_BOTH, wo->wo_rusage) : 0;
...
static int wait_task_continued(struct wait_opts *wo, struct task_struct *p)
{
	...
	if (!wo->wo_info) {
		...
		retval = wait_noreap_copyout(wo, p, pid, uid, CLD_CONTINUED,
					     SIGCONT);
		BUG_ON(retval == 0);
	}
   ...
   */
	...
	if (child != NULL && wo->wo_flags & WTRACE_DESCENDENTS) {
		if (current->pid != init_task.pid &&
		    current->virt_start_time == 0 && child != NULL &&
		...
		struct rusage __user *, ru)
{
	...
	if (options & ~(WNOHANG | WUNTRACED | WCONTINUED | __WNOTHREAD |
			__WCLONE | __WALL | WTRACE_DESCENDENTS))
		return -EINVAL;
