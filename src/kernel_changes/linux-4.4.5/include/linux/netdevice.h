...
enum { NAPI_STATE_SCHED, /* Poll is scheduled */
...
struct rx_queue_attribute {
	...
	ssize_t (*store)(struct netdev_rx_queue *queue,
			 struct rx_queue_attribute *attr, const char *buf,
			 size_t len);
};
...
#define XPS_DEV_MAPS_SIZE                                                      \
	(sizeof(struct xps_dev_maps) + (nr_cpu_ids * sizeof(struct xps_map *)))
#endif /* CONFIG_XPS */
 ...
 *
 *	@hw_enc_features:	Mask of features inherited by encapsulating
 *devices
...
static inline int netdev_set_tc_queue(struct net_device *dev, u8 tc, u16 count,
				      u16 offset)
{
	if (tc >= dev->num_tc)
		return -EINVAL;
 ...
 */
static inline u16 skb_tx_hash(const struct net_device *dev, struct sk_buff *skb)
{
...
#define HARD_TX_TRYLOCK(dev, txq)                                              \
	(((dev->features & NETIF_F_LLTX) == 0) ? __netif_tx_trylock(txq) : true)
...
#define netdev_for_each_all_upper_dev_rcu(dev, updev, iter)                    \
	for (iter = &(dev)->all_adj_list.upper,                                \
	    updev = netdev_all_upper_get_next_dev_rcu(dev, &(iter));           \
	     updev; updev = netdev_all_upper_get_next_dev_rcu(dev, &(iter)))
