...
extern char ___assert_task_state[1 - 2 * !!(sizeof(TASK_STATE_TO_CHAR_STR) - 1 !=
...
#define INIT_CPUTIME                                                           \
	(struct task_cputime)                                                  \
	{                                                                      \
		.utime = 0, .stime = 0, .sum_exec_runtime = 0,                 \
	}
...
#define WAKE_Q(name) struct wake_q_head name = { WAKE_Q_TAIL, &name.first }
...
struct task_struct {
   ...
   * thread-specific */
	...
	/* related fields for virtual time management */
...
#ifdef CONFIG_VIRT_CPU_ACCOUNTING_GEN
extern void task_cputime(struct task_struct *t, cputime_t *utime,
			 cputime_t *stime);
extern void task_cputime_scaled(struct task_struct *t, cputime_t *utimescaled,
...
#define PF_MEMALLOC_NOIO                                                       \
	0x00080000 /* Allocating memory without IO involved   \
                                       */
...
extern int do_execve(struct filename *, const char __user *const __user *,
